...
class Palette(MDApp):
    ...
    def on_tab_switch(self, instance_tabs, instance_tab, instance_tabs_label,
                      tab_text):
        self.screen.ids.box.clear_widgets()
        print(tab_text, 'here is the tab texrt')
        x = colors['Red']
        print(x)

        for value_color in x:
    ...
    def on_start(self):
        print(self.screen.ids.android_tabs.ids.layout.children[-1].text)
        self.on_tab_switch(
            None,
            None,
            None,
